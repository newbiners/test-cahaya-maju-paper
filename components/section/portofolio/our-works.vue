    ...
    <section id="our-works" :class="class_bg" class="py-0 pt-13 pb-35 our-works" ref="section">
        <div class="section-content mt-16">
            <div class="container mx-auto relative">
                ...
                <div
                    class="our-works--tags flex flex-row flex-wrap gap-4 justify-center items-center mb-16"
                >
                    <span
                        class="text-4 cursor-pointer fw-500 border border-current border-1.5 px-6 py-3 rounded-full tracking-wider text-nowrap transition-all duration-300 hover:border-primary hover:text-primary"
                        v-for="tag in tags"
                        :key="tag"
                        :class="{
                            '!border-current !bg-primary !text-white':
                                selectedTag === tag,
                        }"
                        @click="selectTag(tag)"
                        >{{ tag }}</span
...
const visibleItems = computed(() => {
    return props.item_our_works.filter((item) => {
